...
export default function GlobalSearch() {
  ...
  const results = useQuery(
    api.search.global,
    query.length > 2
      ? { query, limit: 20, types: selectedTypes }
      : { query: "", limit: 0, types: [] }
  );
      ...
      <div className="p-4 border-b border-gray-800">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-dark-800 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-primary-500"
            autoFocus
          />
          {query && (
            <button
        ...
        <div className="flex gap-2 mt-3">
          <FilterChip
            label="Memories"
            icon={Brain}
            active={selectedTypes.includes("memory")}
          ...
          <FilterChip
            label="Documents"
            icon={FileText}
            active={selectedTypes.includes("document")}
          ...
          <FilterChip
            label="Tasks"
            icon={CheckSquare}
            active={selectedTypes.includes("task")}
        ...
        {query.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm mt-2">
              Searches memories, documents, and tasks
            </p>
          </div>
        ) : query.length < 3 ? (
          <div className="text-center py-12 text-gray-500">
...
function FilterChip({
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${colors[color]}`}
    >
...
function SearchResultCard({ result }: { result: any }) {
  ...
  return (
    <div className="p-3 bg-dark-800 rounded-lg border border-gray-700 hover:border-gray-600 transition-colors cursor-pointer group">
          ...
          <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
            {result.updatedAt && (
              <span className="flex items-center gap-1">
                ...
                {format(result.updatedAt, "MMM d, yyyy")}
              </span>
            )}
